...
여기서 CDS는 **Class Data Sharing**의 약자로, JVM이 클래스 메타데이터를 아카이브 파일에 미리 저장해 두고, 이후 JVM 시작 시 해당 아카이브를 메모리 매핑 방식으로 읽어 여러 프로세스가 공유할 수 있게 해주는 기능이다. 덕분에 startup 시간과 메모리 사용량을 줄일 수 있다. 이후 AppCDS로 확장되면서 애플리케이션 클래스까지 아카이빙 대상에 포함되었고, 람다 프록시 클래스 역시 이 범주에 들어오게 되었다. CDS와 AppCDS에 대한 보다 자세한 소개는 [dev.java의 CDS & AppCDS 문서](https://dev.java/learn/jvm/cds-appcds/)를 참고하자.
