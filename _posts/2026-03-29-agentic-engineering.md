...
당시의 나는 계속 반복하는 것이 낭비라고 생각하였고 직접 코드를 뜯어보며 해결하려 했다. 토큰을 아껴야 했으니까.
...
개발자의 역할은 문제를 직접 푸는 사람이 아니라, 에이전트가 문제를 풀 수 있도록 **올바른 맥락을 제공하고 최종 판단을 내리는 '디렉터'**가 되는 것이다.

### 결과

결국 월 100달러의 Claude Max 플랜으로 업그레이드했다. 흐름을 끊지 않기 위해서, 그리고 에이전트에게 더 많은 것을 맡기기 위해서. 토큰을 아끼느라 흐름이 끊기고, 에이전트가 할 수 있는 일을 내가 직접 하고 있었다면, 그건 도구를 제대로 쓰고 있는 게 아니다.

단순한 지출이 아니라 투자라고 생각하였다. 토큰에 돈을 쓰는 게 아니라, 그 토큰으로 더 큰 아웃풋을 뽑아내는 것이다.
