...
K-DEVCON 프로젝트들에 원래는 GitHub Gemini Code Review를 쓰고 있었다. 앱만 붙여두면 알아서 리뷰를 달아준다. 무료라 신경 쓸 일도 없다. 그런데 어느 날 PR에 리뷰 대신 이런 코멘트가 달렸다.
...
찾아보니 컨슈머(무료) 버전은 2026년 6월 18일에 deprecated 됐다. 7월 17일에 완전히 종료되면서 엔터프라이즈 버전만 남았다. 잘 사용하던 리뷰어가 하루아침에 사라져버렸다.
...
싼 모델을 찾다 보니 GLM이 눈에 들어왔다. 단가만 놓고 보면 후보로 충분하다. 하지만 중국 쪽 모델이라는 점에서 선뜻 손이 가지 않아 결정을 못 하고 미뤄두고 있었다.
...
Claude Code는 "이 툴만 허용"이라고 나열하는 방식인데 Muse Code는 승인 모드(`untrusted` / `on-request` / `never`)와 샌드박스 설정으로 제어한다. 툴 권한 모델이 눈에 띄게 다르다. 세밀한 화이트리스트를 짜던 사람이라면 그대로 옮기긴 어렵고 승인 정책 + 샌드박스 조합으로 다시 생각해야 한다.
...
<small>
단가 출처: [Meta Model API 요금·레이트리밋 문서](https://dev.meta.ai/docs/pricing-rate-limits), [Anthropic 요금 문서](https://platform.claude.com/docs/en/pricing)
</small>
...
아래 두 표는 Meta가 [Muse Spark 1.3 발표 글](https://research.meta.ai/blog/introducing-muse-spark-1-3)에서 직접 공개한 숫자다. 내가 궁금했던 게 여기였다. 모델을 만든 쪽이 낸 자체 측정치라는 점은 감안하고 봐야 한다.
...
"코딩 벤치마크에서 Opus 5를 이겼다"는 헤드라인은 지금 당장 내가 쓸 수 있는 구성의 이야기가 아니다. 실사용 기준으로는 약간 아래, 대신 훨씬 싸다 정도로 보는 게 맞다. 하지만 PR 리뷰라는 용도에서는 그 정도면 충분하다고 판단했다.
...
위 단가표를 표준 티어 기준으로 다시 보면 배수가 토큰 종류마다 다르다. **입력 12.5배, 출력 21.25배, 캐시된 입력 75배**($0.15 → $0.002)다. 코드리뷰는 diff를 읽는 입력 비중이 크니 이 글에서는 입력 기준인 12.5배로 이야기한다. 가장 보수적인 숫자다.
...
리뷰가 뭔가는 잡아내야 확인이 되니 문자열을 자르는 `truncate` 유틸을 하나 만들어 넣었다. 입력이 `null`일 때 그대로 `.length`를 읽는, 티 나게 잘못된 코드였다. 리뷰 하면서 이걸 짚어내는지 보려는 미끼다.

미끼로 넣어둔 null 가드 누락은 정상적으로 잡아냈다. 심각도 배지가 붙은 총평 코멘트 1개와 인라인 코멘트 3개를 남겼다.
...
재미있는 건 총액이다. 입력 2.5M을 contributor 정가($0.10/1M)로 계산하면 $0.25, 출력까지 더해 **350원 안팎**이 나와야 한다. 실제 청구는 94원이다. 입력의 80% 정도가 캐시에서 나온것으로 보인다.

물론 표본이 크지 않고 diff 크기에 따라 편차도 있을 것이다.

### 주말 이틀을 굴려보고

글을 쓴 뒤 주말 동안 계속 돌려봤다. 위 100회는 붙이자마자 찍은 숫자라 표본이 작았는데 이틀치가 쌓이니 그림이 좀 더 잡힌다.

먼저 리뷰어가 얼마나 일했는지부터. 이틀 사이 여러 저장소에 올린 PR이 서른 개 남짓이고 리뷰는 마흔 번 가까이 돌았다. PR을 열면 자동으로 한 번 돌고, 코드를 고친 뒤 `/review`로 다시 부른 게 몇 번 더 있어서 PR 수보다 실행이 조금 많다.

![Meta Model API 대시보드 주말 사용량](/assets/images/2026-09-04-github-actions-code-review-muse-code/meta-model-api-dashboard-weekend.png)
...
호출 하나당 입력 3만 토큰, 출력 545 토큰, 0.87원이다. 100회 표본에서 봤던 0.94원과 크게 다르지 않다. 앞의 숫자가 우연은 아니었던 셈이다.

리뷰 횟수로 나눠보면 앞에서 어림으로 넘어간 것도 맞아떨어진다. 644 requests는 리뷰 한 번에 모델 호출 열예닐곱 번이고, 559원은 리뷰 한 번에 15원 안팎이다. 첫 리뷰를 돌리고 대시보드에서 봤던 그 15원이 이틀치 표본에서도 그대로 나온 셈이다.

입력 토큰이 2.5M에서 19.6M으로 여덟 배 가까이 늘었는데 청구액은 94원에서 559원으로 여섯 배가 됐다. 정가($0.10/1M)로 19.6M을 계산하면 2,700원쯤 나와야 하니 여기서도 캐시가 80% 안팎을 먹어준 셈이다.

이틀 굴리고 559원. 충분히 감당할 만한 금액이다.
