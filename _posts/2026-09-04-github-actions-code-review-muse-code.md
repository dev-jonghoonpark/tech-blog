...
싼 모델을 찾다 보니 GLM이 눈에 들어왔다. 단가만 놓고 보면 후보로 충분했다. 하지만 중국 쪽 모델이라는 점에서 선뜻 손이 가지 않아 결정을 못 하고 미뤄두고 있었다.
...
미끼로 넣어둔 null 가드 누락을 정상적으로 잡아냈고 심각도 배지가 붙은 총평 코멘트 1개와 인라인 코멘트 3개를 남겼다.
