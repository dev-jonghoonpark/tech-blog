...
이틀 사이 여러 저장소에 올린 PR이 서른 개 남짓이고 리뷰는 마흔 번 가까이 돌았다. PR을 열면 자동으로 한 번 돌고, 코드를 고친 뒤 `/review`로 다시 부른 게 몇 번 더 있어서 PR 수보다 실행이 조금 많다.
...
호출 하나당 입력 3만 토큰, 출력 545 토큰, 0.87원이다. 100회 표본에서 봤던 0.94원과 크게 다르지 않다.
