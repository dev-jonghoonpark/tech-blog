---
layout: "post"
title: "PR 코드리뷰 액션을 Claude Code에서 Muse Code로 바꿔봤다"
description: "Gemini Code Assist 컨슈머 버전이 종료돼 Claude Code로 옮겼는데, 이번엔 코드리뷰가 구독 사용량을 갉아먹었다. Meta의 Muse Code로 갈아타며 정리한 것들 — Opus 5와의 단가·벤치마크 비교, 학습에 코드가 쓰이는 contributor 티어를 고른 이유, 만료되는 OAuth 토큰 대신 정적 API 키를 쓰게 된 점, 그리고 리뷰 1회 실측 비용."
categories:
  - "개발"
tags:
  - "GitHub Actions"
...
무료로 쓰던 컨슈머 버전이 종료됐고 코드 리뷰 활동도 전부 중단됐다는 이야기였다. 리뷰어가 하루아침에 사라진 셈이다.
...
contributor 티어가 싼 데는 이유가 있다. 입력과 출력이 **Meta의 모델 학습에 사용된다**. 표준 티어는 학습에 쓰지 않는 대신 12.5배 비싸다. 단순히 "싼 요금제"가 아니라 데이터 거버넌스 문제다.
...
학습에 들어가서 곤란할 게 없는 코드라면, 12.5배 요금을 데이터 보호 명목으로 낼 이유가 없다. 그래서 contributor를 골랐다.
...
리뷰를 돌리고 나서 Model API 대시보드를 봤더니 누적 사용액이 **15원**이었다. 약 $0.01다.

위에서 가정으로 계산했던 contributor 티어 리뷰 1회 비용이 $0.006, 원화로 8~9원 수준이었으니 실측이 두 배 가까이 높다. 자릿수는 맞았다. 같은 리뷰를 Opus 5로 돌렸다면 가정 기준 $0.375, 500원대다.

표본 하나짜리 숫자다. diff가 커지면 입력 토큰이 늘어나니 리뷰마다 편차가 있을 것이고 15원이라는 금액 자체가 대시보드 표시 단위에 가까워서 정밀도를 기대할 수도 없다. 그래도 "월 200회 돌려도 커피 한 잔"이라는 감각은 얻었다.
