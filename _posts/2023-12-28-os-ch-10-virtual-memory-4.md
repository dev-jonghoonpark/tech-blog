---
layout: post
title: 운영체제 10장 - 가상 메모리 (4) - 프레임의 할당, 스레싱, 메모리 압축, 커널 메모리 할당 등
categories: [스터디-운영채제]
tags:
  [
    운영체제,
...
어떤 프로세스가 실제 실행보다 더 많은 시간을 페이징에 사용하고 있다면 스레싱이 발생했다고 한다.
...
이러한 상황에서 정역 페이지 교체 알고리즘을 통해 어떤 프로세스의 페이지인지에 대한 고려 없이 교체를 수행한다.
