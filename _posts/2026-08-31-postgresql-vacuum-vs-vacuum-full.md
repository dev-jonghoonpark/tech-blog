...
나는 주로 MySQL을 사용해온 개발자인데 최근에는 회사 프로젝트로 PostgreSQL을 쓰꼬 있는 중이다.
...
Dead tuple은 구 버전 행 가운데 어떤 트랜잭션의 스냅샷에서도 더 이상 보이지 않게 된 행이다. 공간은 차지하지만 아무도 읽지 않는다.
...
| ---------- | ----------- | ------- | --------- |
| VACUUM 전  | 2735        | 17242   | 120.2ms   |
| VACUUM 후  | **2736**    | 0       | 14.2ms    |
| REINDEX 후 | **276**     | 0       | 5.7ms     |
...
그래도 시간은 85.5ms에서 27.9ms로 줄었다. 블록을 덜 읽어서가 아니라, 죽은 튜플까지 100만 개의 가시성을 판단해야 했던 것이 10만 개로 줄었기 때문이다. 힙을 읽는 쿼리에서도 VACUUM은 도움이 되는데, I/O가 아니라 CPU 쪽에서 도움이 된다.
...
| ----------------------------- | ------------------- | ------------------------ |
| 힙 접근 생략 (visibility map) | **O — 17242 → 0**   | X (오히려 맵이 초기화됨) |
| 인덱스 블록 — 연속 삭제       | O — 2733 → 274      | 추가 이득 거의 없음      |
| 인덱스 블록 — 산발 삭제       | **X — 2735 → 2736** | **O — → 276**            |
| 힙 순차 스캔                  | X — 17242 그대로    | **O — → 1725**           |
...
같은 명령에 같은 삭제량인데 하나는 그대로고 하나는 10분의 1이 됐다. 파일 중간의 빈 페이지는 앞뒤 데이터에 끼어 있어서 잘라낼 수 없기 때문이다.
